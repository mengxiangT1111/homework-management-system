...
const abstractChildren = [
  ...
  p("本报告为第 3 版（全面复核稿）。第 2 版复核了仓库增量提交；本版在此基础上对全部源码重新做了一次系统性盘点与交叉核实——后端一百二十八个接口端点逐条核对、二十四个数据模型逐一清点、三端全部页面与组件精确统计、测试与脚本资产逐个确认，并新增第一章 1.4 节系统规模量化清单，可直接用于申报书研究基础一节。初版至第 2 版记录的关键变化包括：九项安全漏洞的审计与修复、任务待办功能、mengxiangt.top 全站 HTTPS 与 ICP 备案上线、用户隐私保护指引与新手指引材料等。"),
  new Paragraph({
...
const ch1 = [
  ...
  tbl(["模块", "已实现能力", "关键实现落点"], [
    ...
    ["查重检测", "全班 C(n,2) 两两比对、四层算法流水线、五维雷达图详情、任务进度轮询恢复与取消、命名锁防并发重复建任务", "detection_service 目录、PlagiarismCenter.vue、PlagiarismDetail.vue"],
    ...
    ["任务待办", "教师、班主任、班长、学委面向班级发布待办（标题、说明、可选截止），学生完成回执与进度查看，与作业同权限范式", "todoController.js、todoPublisher.js、student 与 teacher 的 Todos.vue"],
    ...
    ["安全与部署", "九项漏洞审计修复（AI 注入围栏、秒传持有证明、COS 前缀围栏、登录枚举防护等）、短时效下载票据、分级限流、全站 HTTPS 与 ICP 备案上线", "middleware/auth.js、utils/assignmentInput.js、chunk_ownerships 模型、nginx.conf"]
  ], [16, 46, 38]),
  h2("1.3  值得写入申报书的技术事实"),
  ...
  p("查重侧：检测算法为四级渐进流水线——感知哈希初筛、ORB 局部特征匹配、OCR 文本 Jaccard 相似度、图结构比对（节点与边统计差异、图同构判定、图编辑距离精确计算及大图近似公式），综合加权以图结构权重为最高（0.50），判定阈值分为可疑与高度可疑两档；全班比对采用上三角去重、结果双向入库，检测服务与主服务间以令牌鉴权并只监听本机。安全治理侧：项目已完成一轮全量代码审计加动态复现的漏洞修复闭环，共修复九项实测漏洞——学生作答围栏隔离防提示词注入、AI 判满分强制人工复核不自动回写、分片上传参数校验前置（非法请求零落盘）、秒传认领漏洞以 chunk_ownerships 持有证明表根治、COS 签名加作业目录前缀围栏、登录枚举与时序侧信道防护等；查重任务创建引入 MySQL 命名锁防止并发重复建任务，统计口径修正为仅计本班在读学生。工程侧：仓库内已包含评分一致性基准脚本（内置平均绝对误差不超过五分、差值不超五分比例不低于百分之八十、皮尔逊相关系数不低于 0.85、重复批改极差不超过十分等合格阈值，用于对照教师人工分）、查重端到端测试脚本、纯函数单元测试与模型数据库列级一致性校验脚本。上述内容构成第四章不足分析的对立面：本项目的问题不是做得不够多，而是做得好但没有被研究和证明。"),
  h2("1.4  系统规模量化清单"),
  p("以下量化指标由全量源码逐项清点得出（统计不含依赖库与构建产物，截至第 3 版复核时点），可直接引用于申报书研究基础一节，也可作为答辩时的完成度证据。"),
  tcap("表 1-2  系统规模量化清单"),
  tbl(["维度", "量化事实"], [
    ["源码规模", "约 3.04 万行：Node 后端 1.16 万行（84 个文件）、Python 检测服务 0.25 万行、网页端 1.09 万行（51 个文件）、小程序 0.55 万行（25 个文件）"],
    ["接口端点", "HTTP 端点 128 个，分布于 15 个路由模块（公开可匿名访问的仅注册、登录、学校下拉、健康检查 4 个）；检测微服务另有 7 个端点，服务间以令牌鉴权"],
    ["页面与组件", "网页端 28 个页面（公共 5、管理员 6、学生 8、教师 9）与 10 个业务组件；小程序 15 个页面加 4 个底部标签，双端共用一套对齐的设计令牌"],
    ["数据模型", "24 个数据模型对应 24 张数据表：核心业务 13 张、AI 批改 8 张、查重任务 1 张、待办 2 张、上传归属与持有证明 2 张（另含 4 个 SQL 迁移脚本沉淀表结构变更）"],
    ["算法资产", "四级查重流水线（感知哈希、ORB 特征、OCR 文本、图同构与图编辑距离）与三层可信批改防线（量规聚合、置信度路由、注入围栏与满分强制复核）"],
    ["可靠性与安全", "双异步队列（批改并发三路、查重串行，心跳续租与僵死回收）；七个维度的接口限流器；两个定时任务；九项实测漏洞修复；进程优雅停机协调"],
    ["测试与工具链", "23 个单元测试用例；5 个端到端或集成测试脚本；1 个评分一致性校准工具（内置达标阈值）；1 个模型与库表一致性校验脚本；1 个 COS 换桶迁移工具"],
    ["文档与物料", "部署指南、安全清单、小程序三版设计方案与开发提示词、用户隐私保护指引（V1.0）、新手指引图文手册两份"]
  ], [18, 82])
];
...
const ch2 = [
  ...
  tcap("表 2-2  SWOT 分析"),
  tbl(["维度", "内容"], [
    ["优势 S", "系统已完成且公网上线运行（域名、HTTPS、ICP 备案），立项起点远高于从零开始；拓扑图查重与可信 AI 批改两项差异化算法资产；三端齐全，完成九项安全漏洞的审计修复闭环；一致性基准脚本等评测设施已就位"],
    ["劣势 W", "无真实教学试点数据；无软著、论文等成果材料；查重准确率与批改一致性未量化；通知推送、OCR 批改等链路存在断点；查重可视化组件被清理待接回"],
...
const ch3 = [
  ...
  pb("创新点四：还原真实校园组织结构的作业收集闭环。", "提出包含学校、班级职务（班长、学委）、课程课代表的多级组织模型，为非任课教师的收集角色提供代发作业、提交进度、未交名单、智能催交（一小时防轰炸）与按人归档打包的完整工作台，并以创建者与归属教师双字段区分代发与本人发布，兼顾权限与责任归属；任务待办功能进一步将同一权限范式（教师、班主任、班长、学委发布，学生完成回执）复用于日常班级事务，形成作业收集之外的第二个协同场景。"),
  h2("3.2  工程亮点清单"),
  p("除创新点外，以下工程细节可作为申报书研究基础或答辩加分项：文件安全体系（密码修改即全端令牌失效的密码版本指纹机制、十分钟时效的 HMAC 短票据下载替代长期令牌暴露、上传归属校验防止冒绑他人文件、路径围栏防目录逃逸、危险扩展名黑名单）；安全治理记录（九项安全漏洞的审计复现与修复闭环、秒传断点兼容的 chunk_ownerships 持有证明表、COS 越权签名前缀围栏、登录枚举与时序侧信道防护）；并发正确性（查重任务创建的 MySQL 命名锁防重复建任务、批改队列心跳续租与僵死回收）；数据资产护栏（模型与数据库列级一致性校验脚本、统计口径仅计在读学生）；大文件分片上传（秒传、断点续传、合并校验、并发分片磁盘防护）；双模存储与换桶迁移脚本；分级接口限流（AI、查重、下载、分片各自独立阈值）；定时任务（截止提醒、废弃分片清理）；测试资产（模板校验与结果解析的纯函数单元测试、查重端到端脚本、评分一致性基准脚本）。"),
  h2("3.3  与同类系统的差异化对比"),
  tcap("表 3-1  差异化能力对比"),
...
const ch4 = [
  ...
  h2("4.3  工程完整度"),
  p("较初版已明显改善：新增模型与数据库列级一致性校验脚本、查重任务命名锁防并发重复建任务、进程优雅关停与统计口径修复，安全升级 SQL 以迁移脚本形式沉淀，环境变量模板化避免密钥入库。仍存的断点：生产编排仍未包含 Python 检测服务（仅透传环境变量，需跨容器以宿主机地址访问），一键部署存在断点；package.json 仍无测试脚本、无持续集成，单元测试覆盖仅限纯函数；无集中式日志、监控与告警；无接口文档（建议 Swagger 渐进补齐）。上述各项不影响演示，但会被工程背景的评审注意到，且补齐成本低，属于性价比最高的整改区。"),
  h2("4.4  实际落地价值"),
  p("系统尚无真实班级的持续使用记录：没有提交率、批改耗时、教师采纳率、学生满意度等任何一手数据，落地价值停留在设计与演示层面——这是落地维度的唯一硬短板。合规与基础设施侧较初版已实质补齐：完成 ICP 备案并以全站 HTTPS 上线（mengxiangt.top），发布用户隐私保护指引（注册强制勾选同意、登录明示、公开页面可查），并配有新手指引图文手册，开展试点的合规与物料基础已就绪。多学校开通仍依赖管理员手工建校建班，无自助流程，SaaS 化仅具雏形。"),
  h2("4.5  论文与结题材料"),
  ...
  tbl(["维度", "核心问题", "对评审的影响", "处置"], [
    ...
    ["功能", "双拓扑可视化组件被清理", "最出彩演示点缺失", "必须补（接回）"],
    ...
    ["落地", "零真实试点（合规与物料已就绪）", "结题实证不足", "必须补（R8）"],
...
const ch5 = [
  ...
  tbl(["编号", "事项", "编码落点与做法", "工作量", "申报价值"], [
    ...
    ["R7", "工程收尾", "已部分完成（列级校验脚本、查重命名锁、优雅关停、环境模板化、文档更新已落地）；余下：检测服务纳入生产 Compose 编排、package.json 补测试脚本并接 CI、接口文档渐进补齐", "1 至 2 天", "工程完整度整改"],
    ["R8", "首个真实试点", "上线与物料已就绪（域名、HTTPS、ICP、隐私指引、新手指引）；余下：选一门图形作业课程与一门文字作业课程、一个班级起步，记录教师批改耗时基线与 AI 辅助后对比、提交率变化、学生问卷", "持续四至八周", "全部实证材料的来源，最优先"]
  ], [7, 16, 43, 10, 24]),
