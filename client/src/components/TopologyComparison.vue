...
async function loadComparison() {
  loading.value = true
  try {
    // 优先用父组件内联的完整图数据；没有图数据（只有得分行）时按需拉取拓扑接口
    let data = (props.resultData && Array.isArray(props.resultData.srcNodes)) ? props.resultData : null
    if (!data && props.assignmentId && props.submissionId && props.comparedWithId) {
      const res = await plagiarismApi.topology(props.assignmentId, props.submissionId, props.comparedWithId)
      data = res.data
    }
    if (!data) return
    ...
    candidateName.value = data.candidateName || data.studentName || '对比图'
